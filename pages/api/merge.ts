...
export default async function NextApiHandler(req: NextApiRequest, res: NextApiResponse) {
    ...
    argsList.push("-map", `[v]`, "-vsync", "2", "-y", "-preset", "faster", `${process.env.PWD}/public/${req.headers.userid}/output-tmp.mp4`)
    ...
    for (let i = 0; i < fileList.length; i++) {
        ...
        if (file.startDelta > 0) {
            clipList_v.push(`select='not(between(t,${timeSoFar},${timeSoFar + file.startDelta}))'`)
            clipList_a.push(`aselect='not(between(t,${timeSoFar},${timeSoFar + file.startDelta}))'`)
        }

        if (file.endDelta > 0) {
            clipList_v.push(`select='not(between(t,${timeSoFar + clip_duration - file.endDelta},${timeSoFar + clip_duration}))'`)
            clipList_a.push(`aselect='not(between(t,${timeSoFar + clip_duration - file.endDelta},${timeSoFar + clip_duration}))'`)
        }
